...
from ..repositories.json_repositories import LastTemplateRepository
from rich import print
from rich.console import Console


class FromLastInterface(AbstractStrategy):
	def __init__(self):
		super().__init__(repository=LastTemplateRepository())

	def build_from(self, value: SettingsModel, console: Console) -> None:
		print('[green]started build from last template...[/green]')

		template = self.repository.get()
		try:
			settings = value
			tmp_path = template['path']
	...
	def update_on_end(self, model: TemplateModel) -> None:
		self.repository.update(template=model)
